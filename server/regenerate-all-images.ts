...
// Load static cases from frontend data file
const casesPath = join(process.cwd(), '../src/data/cases.ts');
...
if (!casesMatch) {
  console.error('❌ Could not parse cases.ts');
  process.exit(1);
}
const propertyCases = eval(casesMatch[1]);
...
async function regenerateCaseImages() {
  ...
  // Print all results at the end for easy copying
  console.log('\n\n📋 COPY THESE URLS TO UPDATE cases.ts:\n');
  for (const result of results) {
    console.log(`// ${result.id} - ${result.address}`);
