...
type ComparisonResponseRow = SavedComparison;
...
async function getHydratedComparison(
  ...
  const result = await client.query<ComparisonResponseRow>(
    `SELECT sc.id,
            sc.name,
            COALESCE(
...
export async function saveComparison(
  ...
  try {
    ...
    const analysisIds = await resolveAnalysisIds(client, ownerContext, propertySlugs);
    if (analysisIds == null) {
    ...
    if (comparison == null) {
      throw new Error('Saved comparison could not be loaded after creation.');
    }
