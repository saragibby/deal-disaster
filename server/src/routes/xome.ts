...
export function createXomeRouter(requireAuth: RequestHandler = authenticateToken): Router {
  const router = Router();
  ...
  router.get('/market-trends', requireAuth, async (req: AuthRequest, res: Response) => {
    const apiKey = XOME_API_KEY();
    if (!apiKey) {
      res.status(503).json({ error: 'Xome API not configured' });
      return;
...
export default createXomeRouter();
